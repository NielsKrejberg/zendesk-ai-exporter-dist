...
// @updateURL    https://raw.githubusercontent.com/NielsKrejberg/zendesk-ai-exporter-dist/main/zendesk-ai-assistant.meta.js
...
(function () {
    ...
    style.textContent = `
      ...
      #zaec-login-overlay{position:fixed;inset:0;z-index:2147483647;display:none;place-items:center;padding:16px;background:rgba(3,19,11,.66);backdrop-filter:blur(5px)}#zaec-login-overlay.open{display:grid}#zaec-login-modal{width:min(420px,100%);padding:22px;border:1px solid rgba(155,229,178,.34);border-radius:14px;background:linear-gradient(145deg,rgba(28,78,53,.98),rgba(12,42,27,.98));box-shadow:0 20px 56px rgba(0,0,0,.48);color:#fff}#zaec-login-modal h2{margin:0;font-size:18px}#zaec-login-modal p{margin:6px 0 17px;color:rgba(255,255,255,.68);font-size:12px}#zaec-login-modal label{display:grid;gap:6px;margin-top:12px;font-size:12px;color:rgba(255,255,255,.88)}#zaec-login-modal input{width:100%!important;min-height:40px!important;padding:10px 11px!important;border:1px solid rgba(155,229,178,.30)!important;border-radius:7px!important;background:rgba(3,25,15,.42)!important;color:#fff!important;-webkit-text-fill-color:#fff!important;caret-color:#fff!important;outline:none!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.035)!important;appearance:none!important;-webkit-appearance:none!important;font:inherit!important}#zaec-login-modal input::placeholder{color:rgba(255,255,255,.38)!important;-webkit-text-fill-color:rgba(255,255,255,.38)!important}#zaec-login-modal input:focus{border-color:rgba(182,230,195,.72)!important;background:rgba(3,25,15,.58)!important;box-shadow:0 0 0 3px rgba(182,230,195,.10),inset 0 1px 0 rgba(255,255,255,.04)!important}#zaec-login-modal input:-webkit-autofill,#zaec-login-modal input:-webkit-autofill:hover,#zaec-login-modal input:-webkit-autofill:focus,#zaec-login-modal input:-webkit-autofill:active{-webkit-text-fill-color:#fff!important;caret-color:#fff!important;-webkit-box-shadow:0 0 0 1000px rgb(17,58,38) inset!important;box-shadow:0 0 0 1000px rgb(17,58,38) inset!important;border:1px solid rgba(155,229,178,.30)!important;transition:background-color 9999s ease-out 0s!important}#zaec-login-error{min-height:18px;margin-top:10px;color:#ffd3c8;font-size:12px}#zaec-login-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:12px}#zaec-login-actions button{padding:8px 12px;border:1px solid rgba(155,229,178,.32);border-radius:7px;cursor:pointer;color:#fff;background:rgba(255,255,255,.08)}#zaec-login-submit{background:#b6e6c3!important;color:#123421!important;font-weight:700}
    ...
    function renderChat() {
        ...
        for (const msg of state.messages) {
            ...
            const body = document.createElement('div'); renderAnswer(body, msg.content || ''); box.append(role);
    ...
    function renderApprovedSolution(solution) {
        ...
        body.className = 'zaec-approved-body';
        body.textContent = solution.suggested_solution || '';
        box.append(title, name, body);

        const checks = Array.isArray(solution.suggested_checks) ? solution.suggested_checks.filter(Boolean).slice(0, 3) : [];
        if (checks.length) {
            const list = document.createElement('ul');
            list.className = 'zaec-approved-checks';
