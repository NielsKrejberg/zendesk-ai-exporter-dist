...
(function () {
    ...
    style.textContent = `
      ...
      .zaec-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:11px 13px;border-bottom:1px solid rgba(148,210,168,.18)}
      .zaec-title{font-size:15px;font-weight:700}.zaec-sub{font-size:11px;color:rgba(255,255,255,.58)}
      .zaec-head-actions,.zaec-tools,.zaec-input-row{display:flex;gap:7px;align-items:center}
      #${APP_ID} button{border:1px solid rgba(255,255,255,.14);border-radius:6px;background:rgba(255,255,255,.08);color:#fff;padding:7px 9px;cursor:pointer}
      #${APP_ID} button:hover{background:rgba(255,255,255,.13)}#${APP_ID} button:disabled{opacity:.45;cursor:default}
      .zaec-tools{padding:8px 11px;border-bottom:1px solid rgba(148,210,168,.14);flex-wrap:wrap}
      .zaec-primary{background:rgba(117,190,139,.20)!important;border-color:rgba(155,229,178,.35)!important}
      .zaec-status{margin-left:auto;color:rgba(255,255,255,.56);font-size:11px}
      .zaec-chat{flex:1;min-height:0;overflow:auto;padding:12px}
      .zaec-msg{margin:0 0 11px;padding:9px 10px;border-radius:9px;white-space:pre-wrap;overflow-wrap:anywhere}
      .zaec-user{margin-left:45px;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.10)}
      .zaec-assistant{margin-right:28px;background:rgba(37,84,57,.54);border:1px solid rgba(148,210,168,.16)}
      .zaec-role{font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:rgba(255,255,255,.48);margin-bottom:4px}
      .zaec-sources{margin-top:8px;padding-top:7px;border-top:1px solid rgba(255,255,255,.10);display:flex;gap:5px;flex-wrap:wrap}
      .zaec-source{display:inline-flex;padding:3px 6px;border-radius:999px;border:1px solid rgba(155,229,178,.22);background:rgba(117,190,139,.12);color:#dff6e6;text-decoration:none;font-size:11px}
      .zaec-empty{padding:20px 12px;color:rgba(255,255,255,.58);text-align:center}
      .zaec-compose{padding:10px;border-top:1px solid rgba(148,210,168,.18)}
      #${APP_ID} textarea{width:100%;min-height:76px;max-height:180px;resize:vertical;border:1px solid rgba(255,255,255,.13);border-radius:8px;background:rgba(0,0,0,.18);color:#fff;padding:9px;outline:none}
      .zaec-input-row{margin-top:7px;justify-content:flex-end}
      .zaec-error{color:#ffd3c8}.zaec-ok{color:#d9f5e2}
    ...
    panel.innerHTML = `
      <div class="zaec-head">
        <div><div class="zaec-title">Zendesk AI Assistant</div><div class="zaec-sub" id="zaec-ticket-label">Open a ticket</div></div>
        <div class="zaec-head-actions"><button id="zaec-settings" title="Configure access token">⚙</button><button id="zaec-close">×</button></div>
      </div>
      ...
      <div class="zaec-compose">
        <textarea id="zaec-input" placeholder="Ask about this ticket…"></textarea>
        <div class="zaec-input-row"><button id="zaec-send" class="zaec-primary">Send</button></div>
      </div>`;
    ...
    function configureToken() {
        ...
        const value = prompt('Enter your ZENDESK_IMPORT_TOKEN from Supabase. It is stored only in Tampermonkey on this browser.', current);
    ...
    async function fetchComments(ticketId) {
        ...
        while (next) {
            ...
            for (const c of d.comments || []) comments.push({
                ...
                author: users.get(Number(c.author_id)) || null,
                body: c.plain_body || c.body || '',
            });
            const more = [true, 'true', 1, '1'].includes(d.meta?.has_more);
            if (!more) break;
    ...
    async function loadCurrentTicket() {
        ...
        return {
            ...
            url: `${location.origin}/agent/tickets/${id}`,
            conversation: await fetchComments(id),
        };
    }

    async function callSupabase(endpoint, body) {
        ...
        if (!r.ok) throw new Error(data?.error || `Supabase returned ${r.status}`);
        return data;
    }
    ...
    async function addCurrentTicketToKnowledgeBase() {
        ...
        try {
            ...
            setStatus(`Added #${ticket.id}: ${row?.chunks || 0} chunks, ${row?.embeddings || 0} embeddings · PII ${sumRedactions(redactions)}`, true);
        } catch (e) {
    ...
    function renderChat() {
        ...
        if (!state.messages.length) {
            el.innerHTML = '<div class="zaec-empty">Ask about the current ticket or compare it with historical Zendesk cases.</div>';
            return;
        }
        el.innerHTML = '';
        for (const msg of state.messages) {
            const box = document.createElement('div');
            box.className = `zaec-msg ${msg.role === 'user' ? 'zaec-user' : 'zaec-assistant'}`;
            const role = document.createElement('div');
            role.className = 'zaec-role';
            role.textContent = msg.role === 'user' ? 'You' : 'Assistant';
            const body = document.createElement('div');
            renderAnswer(body, msg.content || '');
            box.append(role, body);
            if (Array.isArray(msg.sources) && msg.sources.length) {
                const sources = document.createElement('div');
                sources.className = 'zaec-sources';
                for (const s of msg.sources) {
                    const a = document.createElement('a');
                    a.className = 'zaec-source';
                    a.href = s.url || `${location.origin}/agent/tickets/${s.ticketId}`;
                    a.target = '_blank';
                    a.rel = 'noopener';
                    a.textContent = `#${s.ticketId}`;
                    a.title = `${s.subject || ''} · score ${Math.round(Number(s.score || 0) * 100)}%`;
    ...
    function renderAnswer(container, text) {
        ...
        while ((match = re.exec(text))) {
            ...
            const a = document.createElement('a');
            a.href = `${location.origin}/agent/tickets/${match[1]}`;
            a.target = '_blank';
            a.rel = 'noopener';
            a.className = 'zaec-source';
    ...
    function setBusy(value) {
        ...
        $('#zaec-add-kb').disabled = value;
    }

    function setStatus(text, ok = null) {
        const el = $('#zaec-status');
        el.textContent = text;
